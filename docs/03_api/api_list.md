...
このドラフトについて、以下の点を確認させてください。

1. **パスワードリセット (FN-A03) の実装フロー**
* 現状のAPI定義には「管理者による強制リセット (`POST /api/admin/users/{id}/reset-password`)」のみ含めています。
* **ユーザー自身が「パスワードを忘れました」からリセットする機能**も実装しますか？実装する場合、メール送信機能が必要になりますが、ポートフォリオとしては「管理者がリセットする運用」と割り切るのも手です。どうしますか？


2. **RAGの再インデックス (FN-C04, C08) の通知**
* PDF等の解析・ベクトル化は時間がかかるため、APIは「受付完了 (202 Accepted)」を即座に返し、バックグラウンドで処理が進む想定です。
* フロントエンド側で進捗を知るために、ポーリング（定期的に `GET` を叩く）を行う想定で `GET /api/admin/documents/{docId}` にステータスを含める設計にしていますが、これで問題ないですか？


3. **ストリーミングレスポンス**
* `POST /api/threads/{threadId}/messages` は、LLMの回答を一文字ずつ表示するために **Server-Sent Events (SSE)** または **Chunked Transfer Encoding** を使う想定でよろしいですか？（JSONを一括で返すよりUXが良いです）



ご確認いただき、調整が必要な部分があれば教えてください。特になければ、こちらをベースにAPI仕様書 (`docs/03_api/api_list.md`) を正式作成してください。